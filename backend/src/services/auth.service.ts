...
export const authService = {
  ...
  /**
   * Lie le compte de l'étudiant connecté à un compte PARENT existant, via l'email du parent.
   * Initié par l'étudiant lui-même (et non par le parent) pour qu'il garde le contrôle
   * de qui peut voir ses signalements.
   * Lance USER_NOT_FOUND si aucun compte PARENT ne correspond à cet email.
   */
  async linkParent(studentId: string, parentEmail: string) {
    const parent = await prisma.user.findFirst({ where: { email: parentEmail, role: 'PARENT' } })
