...
export const authRoutes = new Elysia({ prefix: '/auth' })
   ...
   * Lie le compte de l'étudiant connecté à un compte PARENT existant via son email.
   * Initié par l'étudiant : c'est lui qui garde le contrôle de qui peut voir ses signalements.
   * Réponses : 200 lien créé | 401 token invalide | 403 réservé aux STUDENT | 404 aucun parent trouvé avec cet email
   */
  .post('/link-parent', async ({ body, bearer, set }) => {
    try {
      const { userId } = requireStudent(bearer ?? '')
      return await authService.linkParent(userId, body.parentEmail)
    } catch (e) {
      const { status, body: err } = handleError(e)
      set.status = status
      return err
